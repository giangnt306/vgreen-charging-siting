...
def make_seed_grid(bbox, step):
    ...
    return [(float(lat), float(lng + (step / 2 if row % 2 else 0)))
...
def main():
    ...
    ap.add_argument("--type", choices=["cs","other","bss"], default="cs",
                    help="cs=VinFast (mặc định), other=hãng khác, bss=đổi pin")
    ap.add_argument("--bbox", nargs=4, type=float, metavar=("LATMIN", "LATMAX", "LNGMIN", "LNGMAX"),
                    help="Vùng quét: lat_min lat_max lng_min lng_max (mặc định: toàn Việt Nam)")
    ap.add_argument("--grid-step", type=float, default=0.20,
                    help="Khoảng cách lưới seed theo độ (mặc định 0.20, ~22 km)")
    ...
    qa_lat = np.array(q_lat); qa_lng = np.array(q_lng); qa_r = np.array(q_r)
    n_queries = 0
    t0 = time.time()

    def save_ckpt():
        json.dump({"q_lat": q_lat, "q_lng": q_lng, "q_r": q_r},
    ...
    with sync_playwright() as p:
        ...
        def record(data, lat, lng):
            ...
            q_lat.append(lat); q_lng.append(lng); q_r.append(R_cov)
            qa_lat = np.append(qa_lat, lat); qa_lng = np.append(qa_lng, lng); qa_r = np.append(qa_r, R_cov)
            new = 0
            for s in data:
                code = s.get("code")
        ...
        for k, (lat, lng, force_query) in enumerate(seeds):
            ...
            data = query_point(lat, lng)
            if data is None:
                failed.append((lat, lng)); continue
            R, new = record(data, lat, lng)
            n_queries += 1
            if n_queries % 10 == 0:
                save_ckpt()
                rate = n_queries / max(time.time() - t0, 1e-9)
                print(f"    q={n_queries}  điểm {k+1}/{len(seeds)}  trạm={len(found)}  "
        ...
        if failed and not (args.max_queries and n_queries >= args.max_queries):
            ...
            for lat, lng in failed:
                ...
                if data is None:
                    still.append((lat, lng)); continue
                record(data, lat, lng); n_queries += 1
                time.sleep(args.sleep + np.random.uniform(0, args.sleep))
            if still:
