...
def _load_stations(aoi):
    ...
    cols = ["station_id", "lat", "lng", "h3_r8", "quality_flags", "operator", "is_operational", "access", "is_primary"]
    ...
    # loại toạ độ bẩn (F4 — tập cờ dùng chung với covered0, xem features/paths.py)
    df = df[~df["quality_flags"].apply(has_dirty_coord)]
    df["tier"] = "T0"
...
def _qa_gate(cand, aoi, R_km, p_hint, max_candidates=CAND_MAX):
    ...
    return all_ok, {
        ...
        "total_demand_core": round(total_demand),
        "checks": checks,
    }


def build(aoi, R_km=R_BASELINE_KM, p_hint=20, strict=True, max_candidates=CAND_MAX, gapfill_q=GAPFILL_TOP_Q):
    ensure_dirs()
    ...
    with open(str(CANDIDATE_SITES).replace(".parquet", "_qa.json"), "w") as f:
        json.dump(
            {"aoi": aoi.to_dict(), **report, "overall": "PASS" if all_ok else "FAIL"}, f, ensure_ascii=False, indent=2
        )
...
def run(args=None):
    ...
    ap.add_argument(
        "--serve-radius-km",
        type=float,
        default=R_BASELINE_KM,
        help=f"bán kính phục vụ R của MCLP để QA (mặc định {R_BASELINE_KM})",
    )
    ap.add_argument(
        "--p-hint", type=int, default=None, help="số trạm dự kiến MCLP chọn (mặc định 20 city / 800 national)"
    )
    ap.add_argument(
        "--max-candidates",
        type=int,
    ...
    ap.add_argument("--no-strict", action="store_true", help="không exit != 0 khi QA FAIL (chỉ cảnh báo)")
