...
Định nghĩa baseline (F5 — đồng bộ P8/E-DQ2, cùng cột canonical với anchor T0):
  ...
  — đúng công thức "cung công khai khả dụng" của known-issues P8/E-DQ2 (19.053 national).

  1. `is_operational` (op_status resolve official-first, P8): chỉ loại `OUT_OF_SERVICE`.
     `MAINTENANCE` **giữ** (quyết định P8: hạ tầng bảo trì vẫn là cung brownfield đa năm)
     — cột `op_status` được export để model tự loại qua cờ nếu muốn.
  ...
  4. Sạch toạ độ (F4, tập cờ dùng chung `features.paths.DIRTY_COORD_FLAGS`):
     placeholder coords tạo **coverage ảo** — cùng cách xử lý với anchor T0.
...
def build(aoi, strict=True):
    ...
    # --- bộ lọc baseline + bộ đếm lý do loại (flag, không xoá ngầm — §7 #5) ---
    mask, reasons = _baseline_mask(df)
    covered0 = df[mask].copy()
    out = covered0[_OUT_COLS].reset_index(drop=True)
    operational_only = df[_operational_only_mask(df)].copy()
    operational_out = operational_only[_OUT_COLS].reset_index(drop=True)
    ...
    report = {
        ...
        "op_status_breakdown": {str(k): int(v) for k, v in covered0["op_status"].value_counts().items()},
    }

    out.to_parquet(COVERED0_SITES, index=False)
    operational_out.to_parquet(COVERED0_OPERATIONAL_SITES, index=False)
    _write_geojson(out, report, COVERED0_GEOJSON, "covered0 — planning baseline (P8)")
    _write_geojson(
        operational_out,
        report,
        COVERED0_OPERATIONAL_GEOJSON,
