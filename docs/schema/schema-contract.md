...
**Nguồn demand chính thức cho MCLP.** Chứa **thành phần thô** theo ô: `pop`, `road_access_m`, `road_len_m`, `road_lane_mw_m`, `road_lane_ar_m`, `road_bridge_m`, `n_poi`, `n_parking`, `n_fuel` (+ `admin_l1_code`, `province_name`, `commune_name`, `commune_kind`).
...
- [X] **Giang:** tầng **OSM POI + road (trắc địa)** của `demand_h3` → `data/interim/osm/osm_demand_components_h3.parquet` (262.054 ô: `n_poi`/`n_parking`/`n_fuel` + `road_access_m`/`road_len_m`/`road_lane_mw_m`/`road_lane_ar_m`/`road_bridge_m` — **E-DQ7b**, QA PASS). Code `src/ev_siting/data/osm/`, xem [crawler-osm.md](../sources/osm.md).
