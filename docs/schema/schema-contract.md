...
- [X] **Giang:** ghép `pop` (WorldPop 2020 constrained, ~100m) → `data/interim/worldpop/worldpop_pop_h3.parquet` (99,63M người / 104.171 ô) → **`demand_h3` thô** `data/interim/demand/demand_h3.parquet` (254.035 ô sau clip lãnh thổ: `pop` + 5 cột `road_*` + 10 cột POI). Code `src/ev_siting/data/worldpop/`, xem [crawler-worldpop.md](../sources/worldpop.md). **Còn lại:** enrich cột admin (`admin_l1_code`, `province_name`, `commune_*`) + chốt tập cột cuối → cập nhật mục 3.
