...
### 🟢 `stations` — 19.507 dòng · **56 cột** (đo 2026-07-31) · tập cung: 19.181 + 326 loại có lý do (`export_supply_report.json`)

> **31/07 — gỡ 5 cột LIVE khỏi `stations`.** `current_type` · `max_power_kw` · `total_power_kw` ·
> `num_connectors` · `connector_types` là **bản sao** của bảng `connectors` (đối soát: 0/19.225 lệch
> trên cả 4 cột scalar). Bản sao không có cổng ⇒ P7 sửa `connectors` nhưng quên `connector_types`,
> để lại **1.579 trạm tự mâu thuẫn**. Nay `connectors` là **nguồn chân lý duy nhất** của tầng LIVE;
> consumer gọi `evcs.connector_rollup.attach()` (một công thức, một chỗ sửa).
> ⚠️ **Tầng ASSET (E-DQ4) Ở LẠI** — `n_guns_installed` 69.174 vs Σ`count_total` 62.924: **không**
> suy được từ `connectors`.
...
| ~~`current_type`~~ *(đã chuyển sang `connectors`)*          | string                | **Tầng LIVE.** `AC`/`DC`/`MIXED` — registry chính thức quyết khi khớp (**P7**: 20-22 kW là DC CCS2); trạm **evcs-only fallback tier 25 kW** (Q5 hợp nhất 30/07 — không có `UNKNOWN`). ⚠️ Nhãn fallback là **xấp xỉ không có bằng chứng chuẩn cắm** (chính lỗi tier của P7) — chỉ chấp nhận cho trạm không khớp registry. ⚠️ Sai ở **531** trạm (đo 2026-07-31) khi cả một loại dòng bị tắt khỏi mảng sống → dùng `current_type_asset` để **phân tầng** (**E-DQ4**) |
| `station_type` · ~~`max_power_kw`, `total_power_kw`~~ *(→ `connectors`)* | string/double         | **Tầng LIVE** — cấu hình ĐANG BÁO CÁO. `total_power_kw` = Σ nameplate **từng súng** ⇒ **phóng đại 1,82×** so với công suất điểm; dùng `site_power_kw` cho công suất điểm (**E-DQ4**) |
| ~~`num_connectors`~~ *(→ `connectors`, = Σ`count_total`)*   | int                   | **Tầng LIVE** — số súng **ĐANG BÁO CÁO** (đối chiếu bảng `connectors`). ⚠️ **KHÔNG** phải số súng lắp đặt: `evsePowers` là mảng trạng thái sống ⇒ đọc thiếu ở **1.568** trạm, `0` ở **282** trạm (đo 2026-07-31). Dùng `n_guns_installed` (**E-DQ4**) |
| ~~`connector_types`~~ *(→ `connectors`, = distinct `connector_label`)* | list<string></string> | Nhãn tier công suất. **Đây chính là cột đã lệch 1.579 dòng sau P7** — nay suy tại chỗ nên không thể lệch |
...
| `config_src`                                                 | string                | `OFFICIAL` 19.243 / `EVCS_LIVE` 7 / `TELEMETRY_BOUND` 1 (chỉ có nhân chứng telemetry = chặn dưới) / `UNKNOWN` 256 (đo 2026-07-31) |
...
| `confidence`, `freshness`, `quality_flags`               | double/list           | **Tín hiệu chất lượng** (P8 flags: `NOT_OPERATIONAL`/`UNDER_MAINTENANCE`/`STATUS_UNKNOWN`/`NON_PUBLIC`/`ACCESS_UNKNOWN`; **E-DQ4** flags, đo 2026-07-31: `CONFIG_TRUNCATED` 1.568 / `CURRENT_TYPE_CORRECTED` 531 / `POWER_CABINET_SHARED` 4.317 / `CONFIG_UNKNOWN` 256 / `CONFIG_LOWER_BOUND` 1) |
...
### 🟢 `connectors` — tầng 2, 1 dòng/nhóm công suất · FK `station_id` · 24.415 dòng · 11 cột (đo 2026-07-31)

> **NGUỒN CHÂN LÝ DUY NHẤT của tầng LIVE (31/07).** `stations` không còn giữ bản sao.
> Cần rollup theo trạm → `evcs.connector_rollup`:
> `num_connectors` = Σ`count_total` · `max_power_kw` = max(`power_kw`) ·
> `total_power_kw` = Σ(`power_kw`×`count_total`) · `current_type` = AC/DC/**MIXED** ·
> `connector_types` = distinct(`connector_label`) *(≤ `num_connectors` — là LOẠI phân biệt,
> không phải một phần tử mỗi súng)*. Trạm không có connector ⇒ `num_connectors = 0`
> (giá trị LIVE **đúng** theo E-DQ4, không phải thiếu dữ liệu) — **282** trạm.
...
### 🟢 `candidate_sites` — tập điểm ứng viên cho MCLP (**16 cột**) · PK: `candidate_id` · **16.686** dòng (T0 12.827 · T1 2.078 · T2 862 · T4 919 — đo 2026-07-31)
...
- [X] **Giang:** crawl nguồn cung từ evcs.vn → raw `data/raw/evcs/` + master interim `data/interim/stations_master_evcs.csv` (**28.625** trạm — snapshot **2026-07-20**, đo 2026-07-31; PK `station_code` unique, 0 orphan, QA WARN không critical). Code `src/ev_siting/data/evcs/`, xem [crawler-evcs.md](../sources/evcs.md). *(Lịch sử snapshot: 28.417 → **28.625** (21-22/07, snapshot đang dùng) → 28.923 (bản 30/07 trên máy có lô 298 trạm probe 29/07 — **vắng trên máy này**) — xem **P6**.)*
- [X] **Giang (22/07):** transform master (`station_code`, CSV) → parquet canonical `stations`/`connectors` (`station_id` `vn-`) đúng schema mục 3 (H3 res 8 tính từ lat/lng, `operator`, `connector_types`/`quality_flags` kiểu list, `confidence`/`freshness`). Output Hive-partitioned theo `province_code`: `data/interim/canonical/stations/` (**19.507** dòng × **56 cột** — đo 2026-07-31) + `.../connectors/` (**24.415** dòng, tầng 2 nổ từ `evse_powers`). Code `src/ev_siting/data/evcs/transform_canonical.py` (`make canonical`). QA: PK unique, 0 orphan FK, `num_connectors == Σ count_total`. **Phạm vi: chỉ trạm sạc ô tô — MẶC ĐỊNH bỏ `BATTERY_SWAP` (9.118 trạm)**, giữ lại được bằng `--keep-bss`. **E-DQ3:** cột admin **đã enrich** từ ranh giới xã VNSDI (**19.453/19.507** có nhãn — đo 2026-07-31) + trọng tài toạ độ. **31/07:** 5 cột LIVE gỡ về `connectors` (61 → **56** cột) — xem `src/ev_siting/data/admin/`.
...
- [X] **Giang (23/07):** crawl **nguồn chính thức VinFast** (vinfastauto.com, first-party) → `data/interim/vinfast_official/` (registry **23.247** trạm + 71.174 connector + admin — đo 2026-07-31). Xây **matcher 2 tầng** (`match_official.py`): `exact_code` (`station_code==store_id`, **19.427** trạm khớp tuyệt đối — đo 2026-07-31, toạ độ lệch ≤0,3 m) + `spatial_fuzzy` (BallTree haversine + rapidfuzz; 3.730 khớp / 5.468 không khớp trên master 28.625). Output `official_xref.parquet`. `transform_canonical` join vào `stations`: thêm cột **provenance** (`provenance`/`official_matched`/`match_method`/`official_store_id`/`match_dist_m`/`match_name_sim`/`official_charging_status`/`official_access_type`) + **định nghĩa lại** `verified` (corroboration first-party) và `confidence` (`0.4·completeness + 0.6·verification` cho trạm VinFast; `completeness` cho trạm ngoài phạm vi). Canonical: **19.432**/19.507 verified, confidence TB **0,995** (đo 2026-07-31). Doc [crawler-vinfast-official.md](../sources/vinfast-official.md).
