...
> Đánh giá ngày **2026-07-24**, cột "Thực tế" **cập nhật 2026-07-30**. So sánh bản schema collaborator gửi với
> tài liệu gốc của dự án: [problem-analysis.md](../problem-analysis.md) · [known-issues.md](../known-issues.md) ·
> [data-layer/overview.md](../data-layer/overview.md).
>
> ⚠️ **Đây là REVIEW MỘT LẦN, không phải tài liệu sống.** Cột "Schema collaborator" là ảnh chụp bản gửi ngày
> 24/07 — **không sửa**. Cột "Thực tế" đã đồng bộ lại ngày 30/07; nguồn chân lý hiện hành là
> [dataset-inventory.md](../data-layer/dataset-inventory.md) (số đo) và
> [data-layer/overview.md §4](../data-layer/overview.md#4-schema-trạng-thái-thực-tế-đo-2026-07-30) (schema).
...
### 1.2 Phân tầng D0 → DF
- **D1 không cần data mới** → đúng, [data-layer/overview.md](../data-layer/overview.md) xác nhận pipeline cung+cầu đã xong cơ bản.
...
Schema mô tả đúng tín hiệu OSM `power=substation` là proxy thô (caveat median 3.2km) — nhất quán với `known-issues.md` P5 và [data-layer/overview.md](../data-layer/overview.md) §5 (substation-dist BallTree).
...
**Pipeline thực tế ([data-layer/overview.md](../data-layer/overview.md) §1 & §3) dùng:**
```
...
- `gold/demand_proxy` = `data/interim/demand/demand_h3.parquet` (**31 cột / 255.480 ô** tính đến 30/07 — vượt xa 11 cột của bản gửi, nhưng **vẫn chưa có `demand_weight`**)
...
| Chỉ số | Schema collaborator | Thực tế ([data-layer/overview.md §4](../data-layer/overview.md#4-schema-trạng-thái-thực-tế-đo-2026-07-30)) |
|---|---|---|
| Số dòng | **13.258** | **19.507** |
| Số cột | **34** | **61** (30/07 — E-DQ1/2/3/4 + P7/P8 mỗi cái thêm cột provenance) |
| `operator` | Liệt kê là trường solver tối thiểu | ⚠️ **không phải "bẩn"** mà là **HẰNG SỐ** trên tập cung (1 giá trị phân biệt); tín hiệu chủ sở hữu nằm trong `name`, chưa parse — [E-DQ5](../issues/e-data-quality/e-dq5-operator-ownership.md) |
| `status` | Solver tối thiểu | ✅ resolve xong 27/07 → `op_status`/`access`/`is_operational` ([P8](../issues/c-master-data/p8-status-access.md)); `status` thô giữ lại làm nguồn evcs |
...
> [!IMPORTANT]
> Con số 13.258 của collaborator vs 19.507 thực tế — lệch **~6.249 dòng**. Có thể collaborator đang tính sau khi lọc bỏ BSS (9.118 battery-swap) và một số subset, nhưng **con số không khớp với bất kỳ mốc nào trong [data-layer/overview.md](../data-layer/overview.md)**. Cần xác nhận nguồn gốc con số này.
...
**Thực tế (30/07):** `demand_h3` có **31 cột / 255.480 ô** — `pop` + `pop_adj` + 5 cột `road_*` + 3 cột lối vào/bậc
+ **10 cột POI theo lớp tag** + `cell_state`/`frac_in_vn` + **7 cột nhãn hành chính**. ⚠️ `n_poi`/`n_parking`/
`road_len_mt_m` **đã khai tử** ([E-DQ7b](../issues/e-data-quality/e-dq7b-road-semantics.md) ·
[E-DQ7c](../issues/e-data-quality/e-dq7c-poi-taxonomy.md)) — schema collaborator đang khai báo những cột **không còn tồn tại**.
**Vẫn chưa có `demand_weight`** ([E-DQ7d](../issues/e-data-quality/e-dq7d-demand-proxy-validation.md), Kỳ).

| Vấn đề | Chi tiết |
|---|---|
| Số ô lệch | 316.526 vs **255.480** — bản gửi đếm cả ô ngoài lãnh thổ VN ([E-DQ7a](../issues/e-data-quality/e-dq7a-poi-outside-vn.md) clip 268.404 → 254.035; [E-DQ8b](../issues/e-data-quality/e-dq8b-roadless-reallocation.md) thêm lại ô nhận dân) |
...
| `n_parking`, `n_fuel` không khai báo | `n_fuel` có thật; **`n_parking` đã khai tử** → `n_parking_off` + `n_parking_street` (E-DQ7c) |

> [!NOTE]
> Chỗ đỗ và trạm xăng trong `demand_h3` quan trọng vì là input cho **candidate site T1** (parking/fuel = anchor ưu
> tiên). Schema collaborator bỏ qua chúng — và tên cột đúng hiện nay là `n_fuel` · `n_parking_off` ·
> `n_parking_street` (⚠️ `n_parking_off` recall chỉ **8,6%**, thiên lệch đô thị 2,67 — E-DQ7c).
...
Tìm trong [data-layer/overview.md](../data-layer/overview.md): **không thấy bảng này**. Đây là bảng *mới cần xây* — không phải bảng đã có. Schema collaborator ghi ở "Tầng 0 — ĐÃ CÓ" nhưng thực tế **chưa có**.
...
`gold/core` (bundle per-city, sinh bởi `gold_core.py`) — script `gold_core.py` **không có trong codebase** ([data-layer/overview.md](../data-layer/overview.md) §2 không liệt kê). Đây là bảng *cần xây* cho 4 TP pilot (HN/HCM/DN/QN).
...
| **P6** Trùng PK / dedup | ❌ Không nhắc | ✅ đã đóng 24/07 ([P6](../issues/c-master-data/p6-duplicate-pk.md)); trùng **chéo nguồn** là [E-DQ2](../issues/e-data-quality/e-dq2-crosssource-dedup.md) (đóng 27/07) |
...
| **P8** Null status/is_public | ❌ Không nhắc — nhưng `status` đưa vào "solver tối thiểu" | ✅ đã đóng 27/07: dùng `op_status`/`access`/`is_operational` thay cho `status` thô |
| **P9** Lệch thời điểm crawl | ❌ Không nhắc | |
| **P10** WorldPop 2020 lỗi thời | ❌ Không nhắc | |
| **P11** Tổng dân số vs sở hữu ô tô | ❌ Không nhắc | |
| Tọa độ placeholder | ❌ Không nhắc | Ảnh hưởng `lat/lng` solver → ✅ đã đóng 28/07 ([E-DQ1](../issues/e-data-quality/e-dq1-coord-placeholder.md)): 38 loại cứng + `coord_resolved` gate cung |
...
8. ~~Bổ sung `road_len_mt_m` (đường chính) vào demand schema~~ → **cột này đã khai tử 28/07** (E-DQ7b): dùng
   `road_lane_mw_m` (cao tốc) + `road_lane_ar_m` (trục đô thị), đo bằng **lane-mét**.
...
---------------------|-------------------|-----------------|----------
gold/stations        | ✅ Có (19.507×61) | Có (13.258×34)  | ⚠️ Số lệch
gold/demand_proxy    | ✅ 31 cột/255.480 | Có (11 cột)     | ⚠️ Tên cột lệch (n_poi/n_parking/road_len_mt_m khai tử) + demand_weight chưa build
