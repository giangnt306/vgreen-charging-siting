...
| **Toạ độ placeholder** *(mới phát hiện)*            | 35 trạm chồng 1 điểm HCM nhưng địa chỉ ở HN; 274 toạ độ trùng | Consumer T0/covered0 lọc `COORD_INVALID`/`COORD_PLACEHOLDER`/`DUP_COORD_SUSPECT` qua vocabulary chung (F4); policy E-DQ1 v2 audit raw/official trong canonical. |
