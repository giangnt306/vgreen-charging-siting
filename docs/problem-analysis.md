...
> **Thay đổi do mất dữ liệu công ty:** những bộ trước đây xếp Nhóm 1 nhờ "công ty cung cấp" nay được rà lại:
>
> - Bộ **vẫn còn nguồn công khai thay thế** (tọa độ/cấu hình trạm từ crawl evcs.vn/PlugShare; CapEx từ báo giá NCC) → **giữ ở Nhóm 1 nhưng đổi nguồn + hạ độ tin cậy**.
> - Bộ **chắc chắn không có nguồn công khai** (log vận hành nội bộ của VGreen) → **loại khỏi Nhóm 1**, chuyển xuống Nhóm 2 và đánh dấu là rủi ro chặn.
...
> **Đã loại khỏi Nhóm 1 — Dữ liệu vận hành (Logs):** kWh theo giờ/ngày, thời gian sạc/phiên, cao điểm, session start/end, SoC. Đây là **log nội bộ của VGreen, KHÔNG có bất kỳ nguồn công khai thay thế nào** → bị loại khỏi bảng "Có sẵn" và chuyển xuống Nhóm 2 (mục #1) như **rủi ro chặn**. Hệ quả: **không còn phân tích bottleneck/underutilized từ dữ liệu thực** — insight này chỉ khôi phục được nếu sau này công ty mở log, hoặc thay bằng **mô phỏng nhu cầu (simulation)** dựa trên proxy công khai (kém chính xác hơn).
...
| - | ----------------------------------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---------------------------------------------- | --------------------------- |
| 1 | **Log vận hành VGreen** *(mới rớt từ Nhóm 1)* | Bottleneck / underutilized, kWh & giờ cao điểm thực         | **Không có nguồn thay thế.** Chỉ 2 hướng: (a) đàm phán xin lại log từ công ty; (b) **thay bằng mô phỏng nhu cầu** từ POI + dân cư + traffic (ước lượng, không phải số thật)                            | Thấp     | Thấp (nếu mô phỏng)                        | A —**rủi ro chặn** |
| 2 | **Di chuyển (GPS/Traffic)**                          | Lộ trình phổ biến, điểm dừng đỗ lâu                   | Traffic API công khai/thương mại:**Mapbox / HERE / TomTom** (trả phí, có free tier); **Google Roads/Distance Matrix**; định tuyến trên **OSM (OSRM/Overpass)**; xin hợp tác Grab/Be. Uber Movement đã ngừng  | TB        | TB                                             | A                           |
| 3 | **Phân bố xe điện (EV penetration)**              | Quy mô nhu cầu theo địa bàn                                | **Proxy công khai:** thống kê đăng ký xe của **Cục CSGT / Cục Đăng kiểm** theo tỉnh; báo cáo **VAMA/VAMM**, tin doanh số VinFast; suy giảm xuống cấp phường bằng tỉ trọng dân số/thu nhập         | TB        | Thấp–TB (chỉ cấp tỉnh, phải nội suy)    | A                           |
| 4 | **Lưới điện (Grid constraints)**                  | Khả năng cấp điện trạm biến áp cho DC nhanh             | EVN**không public**. Proxy: vị trí trạm biến áp/đường dây từ **OSM (`power=substation`, `power=line`)**; ước lượng công suất theo cấp điện áp; xác minh mẫu bằng khảo sát điện lực địa phương | Thấp–TB | Thấp                                          | A                           |
| 5 | **OpEx phi-điện**                                   | Nhân công, bảo trì, thuê mặt bằng                        | **Khảo sát thị trường:** đơn giá nhân công/bảo trì theo định mức; giá thuê từ trang BĐS (batdongsan.com.vn, chotot) theo khu vực; benchmark ngành                                                                   | TB        | TB                                             | B                           |
| 6 | **Dữ liệu đối thủ**                              | Vị trí & công suất trạm bên thứ ba → tránh chồng lấn | **Crawl:** evcs.vn, PlugShare, Google Maps Places, OSM `charging_station`. Chuẩn hóa & khử trùng lặp                                                                                                                              | Cao       | TB (thiếu công suất/tình trạng real-time) | B                           |
| 7 | **Chi phí & tình trạng mặt bằng**                | Giá thuê, diện tích khả dụng → ràng buộc chọn điểm  | Crawl trang BĐS + lọc theo diện tích/mặt tiền; đối chiếu ảnh vệ tinh/Street View; khảo sát thực địa điểm shortlist                                                                                                           | TB        | TB                                             | B                           |
| 8 | **Biến động mùa vụ / thời gian**                | Nhu cầu lễ tết, cuối tuần, du lịch (trạm cao tốc)       | **Nguồn công khai:** thống kê lượt khách du lịch (Cục Du lịch, Sở VHTT&DL tỉnh); lịch lễ tết; Google Popular Times làm proxy; giả định hệ số mùa vụ                                                               | Cao       | TB                                             | C                           |

> **Đọc bảng:** Ưu tiên xử lý các dòng **(A)**. Trong đó **#1 (log vận hành)** là rủi ro chặn cần chốt hướng ngay (xin lại log hay chấp nhận mô phỏng); **#4 (lưới điện)** khả thi thấp + chính xác thấp → nên chuyển thành **ràng buộc lọc thô** thay vì đầu vào định lượng chính xác. Mọi bộ ước lượng phải ghi vào **assumptions log** kèm biên sai số.

---

## 3. Roadmap theo Sprint (phân công Giang × Kỳ)

Đây là project 2 người, chạy trong **3 sprint**. Khác với giả định cũ (model do "team DS/OR" ngoài đảm nhiệm), **model tối ưu do Kỳ trực tiếp làm**. Phân vai tổng quát:
...
> **Lịch đánh giá (Sprint Review):** đánh giá vào **thứ 7, 2 tuần/lần**. Mỗi task dưới đây có deadline nội bộ để kịp buổi review cuối sprint.
>
