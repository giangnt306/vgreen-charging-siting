...
> **⚠️ Cập nhật 21/07/2026 — Giang tự xây toàn bộ data layer:** dataset gold `30_gold` của Kỳ (snapshot `2026-07-16`) bị lỗi → **bỏ hẳn, không dùng nữa**. Thay vì chờ Kỳ, **Giang tự crawl + xây lại toàn bộ tầng dữ liệu từ đầu** (station/connector + demand map từ WorldPop/OSM/POI + coverage). **Kỳ chuyển sang chỉ lo model (MCLP) + report.** Schema & format vẫn giữ nguyên làm _mục tiêu_ cho bản Giang tự xây (xem [SCHEMA_CONTRACT.md](SCHEMA_CONTRACT.md)). Các mục dưới đây mô tả *nguồn/vai trò* của từng bộ dữ liệu — nay đều thuộc phạm vi Giang tự thu thập.
...
| - | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| 1 | **Chi phí hạ tầng — CapEx**                | Chi phí thiết bị (trụ sạc AC/DC, bộ điều khiển, phần mềm), thi công, đấu nối lưới, trạm biến áp | **Đổi nguồn:** báo giá nhà cung cấp thiết bị + tham khảo vcharge.vn, evcs.vn                                                                        | Độ tin cậy hạ một bậc → dùng khoảng (min–max) thay vì con số cố định |
...
| 3 | **Tọa độ trạms**                           | Xác định khoảng cách địa lý, độ phủ, tránh chồng lấn                                                  | **Đổi nguồn:** crawl evcs.vn, PlugShare, Google Maps Places API, OSM (`amenity=charging_station`)                                                        | Lat/long crawl được cho phần lớn trạm công khai; có thể thiếu trạm mới  |
| 4 | **Cấu hình trạm**                           | Số lượng súng sạc, loại AC/DC, công suất (30/60/150/250 kW)                                                 | **Đổi nguồn:** crawl evcs.vn / app + tag OSM (`socket:*`, `capacity`)                                                                                  |                                                                                     |
...
> **Điểm bàn giao (2 điểm chạm interop — xem [SCHEMA_CONTRACT.md](SCHEMA_CONTRACT.md)):**
>
...
| -- | ------------------- | ----------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ☐ | **21/07**     | Kỳ + Giang | **Chốt schema data + format bàn giao** *(việc chặn — làm trước tiên)* | **In:** yêu cầu bài toán + 2 điểm chạm interop (demand proxy, GeoJSON), mô hình **2 tầng `station → connector`.** <br /><br />**Out:** tài liệu schema thống nhất, đơn vị lưới **H3 res 8** — "hợp đồng" giữa 2 người → **[SCHEMA_CONTRACT.md](SCHEMA_CONTRACT.md)**      |
| ☐ | **17→21/07** | Giang       | Khảo sát nguồn dân số / POI / OSM cho demand proxy **tự xây**                      | **In:** khu vực mục tiêu (1 thành phố). <br /><br />**Out:** danh mục nguồn khả dụng (WorldPop, OSM Geofabrik, POI) + đánh giá độ phủ/chất lượng + phác thảo **công thức trọng số demand**                                                                                                |
...
| ☐ | **22/07**     | Giang       | **Tự crawl `station`/`connector`** từ đầu (thay dataset lỗi của Kỳ) + data doc   | **In:** nguồn crawl (evcs.vn / PlugShare / OSM / Google Places) + schema đã chốt ([SCHEMA_CONTRACT.md](SCHEMA_CONTRACT.md)). <br /><br />**Out:** dataset `station`/`connector` parquet (snapshot Giang tự tạo, đúng schema mục tiêu) + data doc mới do Giang viết |
