...
| **F7**     | F     | `match_official`: NaN distance→`verified=True`; matcher không được wire vào pipeline                                                  | 🟠   | FIX                   | Giang       | **2026-07-28** | ☑           |
| **F8**     | F     | Enumerate resume mất force-seed → under-coverage cụm dày đặc                                                                          | 🟠   | FIX                   | Giang       | **2026-07-28** | ☑           |
| **F9**     | F     | Overpass trả 200 + kết quả cụt (`remark`) không kiểm → POI thiếu ở ô dày                                                             | 🟠   | FIX                   | Giang       | **2026-07-28** | ☑           |
| **F10**    | F     | Bàn giao 21/07 = snapshot mồ côi (hash ≠ MANIFEST, cùng nhãn 07-20, thiếu mọi fix P6–E-DQ2) → cần re-handoff                        | 🟠   | FIX                   | Kỳ+Giang   | **2026-07-28** | ☑           |
...
| **F12**    | F     | Không atomic/fail-fast: `run_pipeline.sh` chạy tiếp khi crawl fail; ckpt ghi thẳng; `transform_canonical` rmtree                      | 🟡   | FIX                   | Giang       | **2026-07-28** | ☑           |
| **F13**    | F     | Ngưỡng 25kW còn 4 chỗ (master + fallback canonical); `num_ports`=totalCharging (74% max-concurrent > tot)                            | 🟡   | FIX                   | Giang       | **2026-07-28** | ☑           |
| **F14**    | F     | `penalty` chuẩn hoá per-AOI trên file dùng chung; `built_up<0.05`/`road=0` hard-exclude (liên E-DQ7/E-DQ8)                           | 🟡   | SIMPLIFY              | Giang       | **2026-07-28** | ☑           |
| **F15**    | F     | Docs/config trôi: schema-contract thiếu 6 cột E-DQ2 + QA ảo; `params.yaml` mồ côi (R=500m); cột chết trong candidate                 | 🟡   | FIX                   | Giang       | **2026-07-28** | ☑           |
| **F16**    | F     | Test né logic rủi ro: không test `_load_stations`/`_gapfill`/`_qa_gate`/covered0                                                      | 🟡   | FIX                   | Giang       | **2026-07-28** | ☑           |
| **F17**    | F     | `_tos_firewall` (phía Kỳ) chỉ bọc bảng `stations`, các bảng dist khác không qua firewall                                              | 🟡   | FIX                   | Kỳ         | **2026-07-28** | ☑           |
| **F18**    | F     | Gộp Low: `fetch_locators` không so `meta.count`; PBF/TIF thiếu check `got<total`; manifest hỏng→WARN; `merge_catalog` ghi vào raw/   | ⚪   | FIX                   | Giang       | **2026-07-28** | ☑           |
| **F19**    | F     | Occupancy sampling theo sự kiện không đều (median 0,9' · max gap 71h) → calibrate P1/A2 bắt buộc duration-weight                     | 🟡   | FIX + DOC             | Kỳ         | **2026-07-28** | ☑           |
| **F20**    | F     | Code sinh snapshot HF 28/07 **mới hơn git HEAD** (cột `coord_*`, cờ `COORD_PLACEHOLDER`/`COORD_ADDR_MISMATCH`, h3 null) — E-DQ1 làm ngoài repo | 🟠   | FIX                   | Giang       | **2026-07-28** | ☑           |
...
- **F4 — bộ lọc dirty-coord chết.** `build_candidates.py:53` & `build_covered0.py:47` lọc `{DUP_COORD, COORD_ADDR_MISMATCH}` — không producer nào sinh. Cờ thật đang tồn tại: `DUP_COORD_SUSPECT` (214 trạm, `dedup_crosssource.py:199-201`) không ai đọc; T0 còn bypass buildable (`build_candidates.py:225`). Fix tối thiểu: thêm `DUP_COORD_SUSPECT` (+cân nhắc `COORD_INVALID`) vào `_DIRTY_COORD_FLAGS` cả 2 file; sửa `candidate-sites.md` đang tuyên bố lớp lọc này hoạt động.
  - **☑ Fix 28/07:** tập cờ dùng chung `features/paths.py:DIRTY_COORD_FLAGS = {COORD_INVALID, COORD_PLACEHOLDER, DUP_COORD_SUSPECT}` + helper `has_dirty_coord` — khớp producer v2, cả 2 consumer import chung. `COORD_ADDR_MISMATCH` là cờ lịch sử, không phải output policy v2. Full test xác nhận T0/covered0 dùng cùng vocabulary.
- **F11 — new_supply phía Kỳ nhiễm (chặn B4).** `evcs-dataset/src/evcs/transform/evcs_vn_new_supply.py`: không dedup nội tập NEW (121/3.797 row là cặp <50m; **40/1.544 tier HIGH** — chính ground truth T1); 46 OutOfService + 659 Maintaining pass `ever_active==1`. Fix trước B4: dedup nội tập (coord<50m + name-sim + blob-guard, mượn `dedup_crosssource`) + cờ trạng thái từ canonical mới của Giang qua `station_code`.
  - **☑ 28/07 (repo `evcs-dataset`):** status gate + dedup nội tập chạy trên handoff v4; smoke mới nhất: 3.806 ever-active → 3.098 operational/public/primary unique, NEW 3.093, DRIFT_DUPLICATE 5.
...
- **F5 — covered0 lệch hệ quy chiếu.** `build_covered0.py:44-52,73-76` dùng `status`/`is_public` thô, không đọc `op_status`/`access`/`is_operational`/`is_primary` → 329 dup đếm 2 lần, trạm official-ACTIVE bị loại oan, T0 ⊄ covered0. Fix: filter = `is_operational & access=='PUBLIC' & is_primary` (công thức 19.053 của P8/E-DQ2); lý tưởng dùng chung `_load_stations` với candidates.
  - **☑ Fix/rebuild 28/07:** filter dùng `is_operational & access=PUBLIC & is_primary` + dirty-coordinate shared gate; export dùng `op_status`/`access`, không dùng raw status. National canonical v2: planning baseline **18.839** (= 19.507 trừ 42 OOS, 22 restricted, 67 unknown access, 329 duplicate, 214 dirty); operational-only sensitivity **15.628**. `MAINTENANCE` được giữ trong planning baseline theo P8.
...
- **F7 — match_official.** (a) `match_official.py:249-250`: `pd.isna(dist) or ...` → sửa thành `pd.notna(dist) and ...`, thiếu toạ độ = mức "code-only" riêng; (b) không có `make match`, transform fallback im lặng khi thiếu xref (`transform_canonical.py:246-249`) → thêm target + FAIL/WARN khi xref thiếu/stale (mất xref = E-DQ2 T1 biến mất im lặng).
  - **☑ Fix 28/07:** exact-code thiếu một trong hai toạ độ thành `exact_code_no_coord`, `verified=False`; only exact-code có distance hữu hạn ≤200m mới verified (dữ liệu 07-20 có **0** dòng dạng này → không đổi số, đây là chốt chặn cho tương lai). `make canonical` gọi `make match-official`; matcher ghi SHA-256 master vào xref, canonical FAIL nếu xref thiếu/stale/trùng/không phủ đủ `station_code`. Recovery phải gọi tường minh `--allow-missing-xref`.
  - **Sửa cổng coverage 28/07 (review):** cổng ban đầu đòi `set(master) == set(xref)`, nhưng matcher chạy trên master **đầy đủ** (28.625, có BSS) còn `join_xref` chạy **sau** khi canonical bỏ BSS (19.507) → `make canonical` **FAIL 100%** ở đường mặc định. Đổi sang **phủ (subset)**: `set(canonical) ⊆ set(xref)`, báo rõ số mã thiếu. Cổng SHA-256 vẫn giữ nguyên tính chặt (cùng file master ⇒ cùng tập mã). Regression: `test_f7_gate_accepts_car_only_subset_of_full_master_xref`.
- **F8 — resume mất force-seed.** `evcs_enumerate.py:296-299,364-377`: resume không tái tạo force-seed cho trạm đã phát hiện → cụm >50 trạm không được mở rộng tiếp. Fix: khi resume, re-seed mọi trạm trong `found` chưa nằm trong đĩa phủ.
  - **☑ Fix 28/07:** `_resume_force_seeds()` tái thêm đúng các trạm đã tìm nằm ngoài mọi đĩa coverage; seed được đánh `force=True`. Coverage không còn nới 1,05×R và response thưa (<50) không được quyền che seed — tránh skip cụm dày hoặc kết quả partial.
  - **↩ Sửa lại 28/07 (chiều thưa) — `R_cov = R` cả hai nhánh.** Hạ bán kính phủ về 0 khi response <50 là **bảo thủ sai chỗ**, và đo được: resume toàn quốc ≈ **9,5h** vì gần như mọi seed vùng thưa phải query lại, mà không phát hiện thêm trạm nào. Lập luận: `len < 50` nghĩa là **cap không ràng buộc** ⇒ server đã trả hết những gì nó có quanh điểm đó, nên đĩa `max(dist)` phủ đầy đủ — kết luận này còn *không cần* giả định sắp xếp theo khoảng cách mà nhánh cap=50 phải dựa vào, tức là **chắc hơn** chứ không yếu hơn. Rủi ro thật (cụm >50 trạm bị cắt) do force-seed ở trên xử lý, không phải do hạ `R_cov`. Quy tắc tách thành hàm thuần `coverage_radius()` (`evcs_enumerate.py`), test `test_f8_sparse_response_proves_the_same_coverage_disc_as_a_capped_one`.
- **F9 — Overpass truncation.** `overpass_poi.py:71-72`: HTTP 200 + `remark` "runtime error/timed out" được nhận nguyên. Fix: check `remark` → ép tách bbox như nhánh ≥40k.
  - **☑ Fix 28/07:** HTTP 200 chỉ được chấp nhận khi JSON là object hoàn chỉnh, không có `remark`; malformed/incomplete response thành `RuntimeError` để quadtree split + retry, không biến thành POI rỗng.
- **F10 — re-handoff snapshot chuẩn.** Bản trong `evcs-dataset/00_raw/source=evcs_vn` (28.417, copy 21/07) khác sha256 với MANIFEST frozen (28.625, crawl 21-22/07) — cùng nhãn `2026-07-20`, thiếu mọi fix P6–E-DQ2. *Cập nhật 28/07: snapshot HF (bit-identical với MANIFEST, verify `--hashes` PASS) đã tải về `aGiang-evcs/data/` → re-handoff từ đây: catalog frozen + canonical (`op_status`/`is_primary`/`physical_id`/`connector_standard`), drop cột bẫy `gold_station_id` (NN không ngưỡng, p99 6,6km).*
  ...
  - **☑ Re-handoff v4 28/07:** bundle bất biến `handoff=2026-07-28-v4` gồm catalog 28.625 dòng, telemetry 18.630.532 row, canonical stations 19.507/connectors 24.415. `HANDOFF.json` hash raw/source manifest/canonical trees **và** ghi revision + dirty-state + coordinate policy sinh canonical (28/07 bổ sung neo cho catalog interim — xem **F18**). Consumer đã rebuild raw → bronze → Gate-2 → new-supply trên v4: 19.427 CS, 19.217 có telemetry, NEW 3.093.
...
- **F18 — gộp Low.** **☑ Fix 28/07:** locator fail nếu `meta.count` khác items; PBF/TIF fail nếu Content-Length không khớp; manifest corrupt = CRITICAL; merge catalog/code list ghi `data/interim`. *(Mục "sparse response không che coverage seed" đã bị **đảo lại** — xem F8 ↩.)*
  - **Hệ quả của việc chuyển catalog sang `data/interim/` — đã đóng 28/07.** MANIFEST frozen chỉ phủ `data/raw/`, nên sau F18 catalog bàn giao **rời khỏi vùng được verify**: `export_handoff` băm bản copy trong bundle, nhưng không gì nối bản copy đó về file producer. Nay `HANDOFF.json.catalog.source` ghi `path` (`data/interim/evcs_catalog.csv`) + `sha256` + `bytes` + `derived_from_manifest_role: evcs/catalog` (role thô đã freeze mà nó dẫn ra) ⇒ chuỗi raw đã freeze → merge → interim → bundle đối soát được đầu-cuối. Thêm cổng chặn bản rách: `merge_catalog` ghi **không atomic** (`open("w")`), nên hash nguồn được đối chiếu **sau** khi copy — lệch thì FAIL thay vì bàn giao im lặng; dọn thư mục tạm đổi sang `except BaseException` để `SystemExit`/Ctrl-C không bỏ lại `.<name>.tmp-*`. Test: `tests/test_handoff_bundle.py`.
