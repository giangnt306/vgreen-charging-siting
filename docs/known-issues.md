...
- **F10 — re-handoff snapshot chuẩn.** Bản trong `evcs-dataset/00_raw/source=evcs_vn` (28.417, copy 21/07) khác sha256 với MANIFEST frozen (28.625, crawl 21-22/07) — cùng nhãn `2026-07-20`, thiếu mọi fix P6–E-DQ2. *Cập nhật 28/07: snapshot HF (bit-identical với MANIFEST, verify `--hashes` PASS) đã tải về `aGiang-evcs/data/` → re-handoff từ đây: catalog frozen + canonical (`op_status`/`is_primary`/`physical_id`/`connector_standard`), drop cột bẫy `gold_station_id` (NN không ngưỡng, p99 6,6km).*
  ...
  - **☑ Re-handoff 28/07:** bundle bất biến `handoff=2026-07-28-v3` gồm catalog 28.625 dòng, telemetry 18.630.532 row, canonical stations 19.507/connectors 24.415. `HANDOFF.json` hash mọi raw + source manifest + canonical trees. Consumer bỏ fallback raw cũ; bronze, Gate-2, new-supply và evidence đọc bundle hash-verified. Rebuild đã chạy: 19.427 CS, 19.217 có telemetry; F11 dùng bundle hoàn chỉnh.
...
- **F20 — code sinh data mới hơn git HEAD (phát hiện 28/07 khi fix F4).** Canonical parquet trong snapshot HF 28/07 chứa cột/cờ mà code tại HEAD `495b0c3` **không sinh ra**: `lat_raw`/`lng_raw`/`coord_src`/`coord_fix_dist_m`/`coord_resolved`, cờ `COORD_PLACEHOLDER` (38, `coord_resolved=False`, **`h3_r8=null`**) và `COORD_ADDR_MISMATCH` (758) — tức E-DQ1 đã được Giang làm một phần **ngoài repo**. Hệ quả: (a) không review/tái lập được bước coord-fix; (b) semantics `h3_r8=null` làm consumer HEAD crash (đã chắn ở F4). Xử lý: Giang push code E-DQ1 + cập nhật schema-contract (cột `coord_*`); tới lúc đó coi các cột `coord_*` là **chưa kiểm chứng**.
  - **⚠ Mất dữ liệu ĐÃ XẢY RA THẬT (28/07, khi review F7/F12).** Chạy `make canonical` một lần đã **xoá sạch** toàn bộ công E-DQ1: `coord_*` biến mất, `COORD_PLACEHOLDER` (38) + `COORD_ADDR_MISMATCH` (758) biến mất, chỉ còn `DUP_COORD_SUSPECT` (214) — hệ quả `covered0` **18.105 → 18.839** (nhận lại 734 trạm toạ độ bẩn vào baseline) mà **không có cảnh báo nào**. Nguyên nhân: `transform_canonical` tại HEAD không sinh các cột đó, còn F7 vừa biến `make canonical` thành một lệnh duy nhất rebuild tất cả. Đã khôi phục từ bundle `handoff=2026-07-28-v3` (bit-identical, 19.507/24.415) và xác nhận `covered0` về lại 18.105.
  - **Rủi ro còn nguyên:** bất kỳ ai chạy `make canonical` trước khi code E-DQ1 được push sẽ **lặp lại** mất mát này. Cho tới lúc đó: (a) coi `data/interim/canonical/` là **artefact không tái lập được**, chỉ rebuild khi đã có bản sao bundle; (b) cân nhắc guard trong `transform_canonical` — refuse ghi đè nếu canonical hiện có chứa cột/cờ mà code hiện tại không sinh ra.
  - **Lỗ hổng trong câu chuyện verify của F10:** `export_handoff` verify MANIFEST (nguồn **thô**) + schema canonical, nhưng **không** chứng minh được canonical trong bundle sinh ra từ code nào. Bundle 07-28-v3 hash-verified vẫn chứa output của code ngoài repo. Đóng hẳn khi E-DQ1 lên git (gợi ý: ghi `git rev-parse HEAD` + dirty-flag vào `HANDOFF.json`).
